...
- 2026-09-09：**T8 实现完成**（PR G，分支 `feat/verify-isolation`，base 在 A 上）。
  **开工第一件事是纠正我自己对这一项的理解**，它直接决定可行性。

  重读设计原文：「起隔离栈（per-task compose project：**动态端口 +
  独立 DB schema**）」+「待目标仓库**声明服务栈**后补」。
  它说的**不是**「把测试进程塞进容器里跑」，而是「给每个任务一套自己的
  **依赖**（自己的库、自己的随机端口），把连接串注入验证命令」。
  测试仍在宿主的 worktree 里跑。

  两种读法的难度差一个量级：容器化测试运行器需要目标仓库镜像自带工具链
  与源码，还要解决 `docker exec` 收集退出码、缓存挂载、权限一堆问题；
  而 per-task 依赖栈复用现成的 `InfraCatalog` + 随机端口 + 标签清理，
  解决的正是**真实痛点** —— 并发任务共用一个 postgres、互相写脏对方的表，
  红绿结论因此不可信。后者才是「验证结论可信」这个产品主张的必要条件。
  真正的**执行**隔离（bubblewrap）是另一件事，roadmap §3.5 第 14 条已单列。

  设计决定：
  - **依赖由仓库显式声明**（`repos.verify_infra`，migration 0019，默认空）。
    不自动推断：目标仓库的 compose 里既有依赖也有应用自身，还可能有前端
    dev server、监控、mock —— 自动推断必然要么起多了（慢且浪费）
    要么起少了（测试连不上）。默认空即无隔离，这是 AC6「隔离不得成为
    硬门槛」的实现。
  - **代码落在 preview 包内**：`Manager.exec` 是私有字段（测试注入点），
    放包外的话 runner 侧单测只能依赖真 docker —— 那种测试在 CI 里第一个挂。
  - **不复用 `Manager.Start`**：它异步 fire-and-forget，且 `ops` 以 taskID
    为键会与人点的预览互相拒绝。验证要同步阻塞、起不来立即报错。
  - **撞车防护**（AC3）：验证栈用独立标签 `lathe.verify`，**绝不打
    `lathe.task`**，项目名走 `ComposeProjectFor("verify", id)`。
    `Manager.Stop` 的两路标签查询因此都匹配不到它 —— 否则人点一次
    「停止预览」会把正在跑的验证容器一起删掉。
  - **必须发布随机端口再回读**：容器内用别名 `pg` 互访，但验证命令跑在
    宿主上解析不了它，得用 `127.0.0.1:<回读端口>`。为此给 `infraSpec`
    加了 `Port` 与 `HostEnv`（宿主口径连接串）。
  - **env 合并让栈的连接串压过宿主同名值**：开发机上常导出过
    `DATABASE_URL` 指向共享库，不覆盖就等于隔离白做。
  - **light 档刻意不起栈**：设计上它就「不起栈」（构建/lint/类型检查
    不连外部依赖），起个用不上的 postgres 只是白烧资源。
  - **AC5 降级显式决策为「回落无隔离 + 留痕」而非排队等待**：机器忙不是
    任务的错，回落仍能给出验证结论；排队则可能让验证槽位无限期挂着，
    因为水位未必会降。但必须留痕 —— 不然人看到的只是一次「通过了」，
    无从知道它其实跑在共享环境上。
  - **AC7 起栈失败有独立错误身份**（`ErrVerifyStackUp`）：若冒充成复现
    阶段的 `StatusError`，会被 `redEnvError` 归类成「环境问题、失败留现场」
    —— 语义上恰好也对，但错误信息会让人去查一个根本没问题的测试命令。
    也不能被 `isReproContractErr` 判为「agent 能自己修」，否则 agent 会被
    派去修一个 docker 问题。
  - 适配器里防了一个 **Go 经典坑**：没声明依赖时返回 `nil, nil` 而不是
    包着 nil 的接口值 —— 后者会让调用方 `stack != nil` 意外为真，
    然后对 nil 指针调方法当场 panic。有测试钉着。

  **又踩了一次 T4 记下过的坑**：几处 `python` 替换没加 `assert`，
  而本分支基于 baseline（没有 PR C 的 `StepLogger`），按 T4 签名写的替换
  **静默失败**了 —— 编译报错才发现。我在 T4 的文档里写过「以后每处替换都要
  有断言」，隔了几轮自己就忘。重做时每处加 assert，一次过。
  **这条教训看来需要的不是记录而是习惯。**

  测试：preview 7 条（含撞车防护、随机端口回读、水位拒绝、半个栈清理）
  + runner 7 条（降级判定、错误身份、nil handle、env 覆盖）。
  其中还修了一次**测试自己的 bug**：断言「所有含 `lathe-verify-t7` 的调用
  都要带标签」，但 `docker exec`（就绪探测）与 `inspect`（回读端口）
  本来就不带。收窄到创建资源的调用后，另加一条防空转断言 ——
  免得循环一次没执行也报「通过」。

  门禁：注入 26 条非终态孤儿行后跑整套，`GOTEST_EXIT=0`、15 包全绿、
  `go vet` 与 `gofmt` 干净、`make ui` 通过。
  migration 用 **0019**（避开 PR D 的 0018；两者编号唯一，合并顺序无关）。

---

## 9. 整轮收尾

九项全部完成（T0–T9），7 个 PR：#4（A）/#5（B）/#6（C）/#7（D）/#8（E）/#9（F）/G。

**这一轮最值得记的不是交付了什么，而是几次「我以为」被现实纠正：**

| 我原先的判断 | 现实 |
|---|---|
| 基线是绿的 | 首轮就红，且我用 `\| tail` 取错退出码误判过一次 |
| 陈尸被全局 `ClaimReady` 领走 | **实验证伪**，真根因是两个不同机制的 fixture 缺陷 |
| `gate_mode` 断点在 pipeline | 在 `Enqueue` —— 它从不把仓库配置复制进任务行 |
| B/C/D/E/F 互不依赖 | C 依赖 B（验收标准之间有引用） |
| `tasks.agent_cost_usd` 是任务成本 | 是**最后一轮 fix** 的单轮成本，被反复覆盖 |
| 首轮 reaper 清掉 9 个遗留目录 | 4 个该清、4 个绝不能清（`pr_open`）、1 个 DB 根本看不见 |
| T8 = 把测试塞进容器 | = 给每个任务一套自己的依赖栈 |

**三类反复出现的自我失误，值得当成习惯而非笔记来治：**
1. **退出码取错**：`cmd \| tail` 取到的是 `tail` 的。共犯 4 次（含 push 那次）。一律用 `$?` 显式捕获或 `PIPESTATUS[0]`。
2. **无断言的替换**：`str.replace` 静默失败，「改了」与「以为改了」分不开。在 T4 记录过，T8 又犯。
3. **绿是不是真的**：`t.Skipf` 也显示 `ok`；单元测试全绿掩盖过一个漏掉的字段（T4 的 `Logs: logs`）。每轮用 `-v` 数 RUN/PASS/SKIP，且端到端断言不可省。

  **合并前审计后的加固（3 条阻断项，全部与「栈起来了没有、拆掉了没有」有关）。**
  标签隔离方案本身经审计核实是**正确**的（`Manager.Stop` 的三处六条过滤器与
  `lathe.verify` 全不相交，反向也不会误删预览容器），命令注入面也干净 ——
  这些没动。
  ① **worker 先跑、Stacks 后装。** `go q.work(ctx)` 在 main.go:153，而
  `pipeline.Stacks = ...` 在 257 —— worker goroutine 读它、main goroutine
  无同步地写它，是 Go data race（interface 值两个字，撕裂读可能读到非 nil 的
  itab 配 nil 的 data，然后在验证主路径上 panic；`go test -race` 抓不到，
  因为 `serve()` 没有测试覆盖）。更实的影响：这两行之间被领走的任务
  `Stacks == nil` → **既不起栈也不留痕**，而这个窗口恰恰是最热的时刻 ——
  150 行的 `Reconcile` 刚把所有在途任务重新入队。原注释意识到了「Manager
  造得晚」这个别扭，但没意识到 worker 已经在跑了。修法刻意不取最小改动
  （把两个 `go` 挪到最后）：那修得了这一次，下一个人在中间插一行 `go xxx`
  就又破了，约束仍只活在注释里。改成 `buildPipeline` 返回 Manager 且
  `Stacks` 写在**构造字面量**里 —— 语言层面拿不到一个 `Stacks == nil` 的
  Pipeline；后台执行者收敛到单一启动点 `startWorkers`，放在所有装配之后。
  ② **用已取消的 ctx 拆栈 = 容器泄漏。** `Manager.exec` 是
  `exec.CommandContext`，ctx 取消后 `docker rm -f` 一条都执行不了；而
  `defer stack.Down(ctx)` 用的正是优雅退出时会被取消的那个根 ctx。
  Ctrl-C 重启一次就永久留下 postgres 容器 + 网络。在 `Down` **内部**
  用 `context.WithoutCancel` 自己解绑，一处覆盖四条失败路径。
  ③ **没有任何 `lathe.verify` 的兜底清扫。** 不打 `lathe.task` 是对的，
  代价是它掉出所有既有清理路径：预览容器泄漏了人还能点停止，验证容器
  泄漏了**没有任何出口**。而容器名是确定性的，于是泄漏一次之后同一任务的
  每次重试都撞 `Conflict. The container name is already in use` → 判死 →
  **任务永久红**，修复回路每轮重跑验证也会重新起栈，`Down` 失败一次就把
  后续所有轮次锁死。补起栈前按精确名字回收（不按标签批量删 —— 那会误删
  同一任务另一个并发栈）+ 开机清扫（放在 `Reconcile` 之后、worker 启动
  之前，此刻不可能有本进程起的栈，无条件全清安全）。

  另有一条顺带项**直接反噬本项的立项理由**，值得单独记：就绪探测走 unix
  socket、探不到 TCP。postgres 官方 entrypoint 首次初始化时先起一个
  `listen_addresses=''` 的临时服务器建库，跑完 initdb.d 后**停掉再以正常
  配置重启**；而探测命令无 `-h`、走的正是 unix socket，它在临时服务器阶段
  就会成功。此时回读端口、验证命令立刻连 `127.0.0.1:<port>`，而 postgres
  正在重启中 → connection refused → 测试红。「让并发任务的红绿结论可信」
  正是 T8 的立项理由，这个偶发红直接抵掉它。补宿主侧 TCP 探活 —— 而不是
  给探测命令加 `-h 127.0.0.1`：后者只让容器内那次走 TCP，仍测不到宿主侧
  端口有没有发布好，而验证命令跑在宿主上。

  原自述有一处不实，如实修正：「项目名走 `ComposeProjectFor("verify", id)`」
  —— 生产代码里验证栈走 `docker run`，根本没有 compose 项目名，那个调用
  只出现在测试里。隔离结论不受影响（不打反而更彻底），但描述与代码不符，
  且那条断言因此是空的。
