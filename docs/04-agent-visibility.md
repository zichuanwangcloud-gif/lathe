...
> 2026-09-09 更新：`cmd/lathe-runner` 骨架已删除（见 08-debt-cleanup.md D8-1）。
> 本节的结论不受影响——它论证的是「独立节点形态**若**落地，本方案不阻碍」，
> 而非依赖那个骨架存在。重启多节点时本节仍是有效的设计约束。

独立节点形态落地后，EventSink 不能直接写控制面 DB，事件需随
节点 → 控制面的上报通道回传（复用心跳/租约连接）。对 API 与前端**完全透明**：
