...
- **AI 推荐**（弹窗「AI 推荐」按钮，不自动跑）：只读 agent（plan 权限
  模式）在 worktree 里分析后建议「起哪个候选、变量填什么」。prompt 带
  三份证据：issue 上下文（Linear 取标题+描述，取不到退化为 key）、候选
  清单（含必填变量）、本机在跑的容器（可复用依赖）。输出严格 JSON 并
  校验：推荐路径必须在候选清单里、变量名必须在扫描结果里 —— 幻觉宁可
  报错也不预填进表单。同任务同 HEAD 缓存结果。走分诊同款便宜通道，
  3 分钟硬上限。**推荐 = 预填，启动仍是人拍板**；找不到证据的变量
  （尤其口令）要求留空说明，不许编造。实测（CR-1454）agent 用
  git show HEAD 定位改动应用、识破同目录 compose 的 build context 必然
  失败、env 逐条标 file:line 证据 —— 比人看 21 个候选文件名猜强得多。
