...
| 阶段 | 范围 |
|---|---|
| **P0 闭环** | 单机单账号串行。状态机 + 持久化 + 幂等；`direct` 档；`light` 验证；失败三件套；~~worktree 自动回收~~（**表述不实**：P0 当时只做了「合并后回收」与「同名尸体按需回收」两个被动触发点，失败/取消的现场只增不减 —— TTL 收割机 2026-09-09 才交付，见 [08-debt-cleanup.md](./08-debt-cleanup.md) T6）。**刻意不做**：多节点、并发、Web UI |
...
- **AI 推荐**（弹窗「AI 推荐」按钮，不自动跑）：只读 agent（plan 权限
  模式）在 worktree 里分析后建议「起哪个候选、怎么连依赖」。分四层：
  1. **机械证据**（Lathe 算，不靠模型猜）：`git diff <base>...HEAD` 得出
     改了哪些应用、**是否有 SQL/迁移变更**；在跑容器清单（DB 类附
     inspect 出的凭据与宿主端口）；候选清单。
  2. **决策**（agent）：prompt 要求先读项目部署文档，按文档推荐；输出
     严格 JSON（候选 + 数据库策略 + env 建议 + infra）。
  3. **安全校验**（Lathe 机械执行）：推荐路径/变量名必须在扫描结果里；
     **有 SQL 变更选 reuse 会被强制纠正为 clone 并留痕**（迁移可能改坏
     共享库，不信模型自觉）；源容器必须真实在跑且是 DB 镜像。
  4. **执行**：reuse = 应用容器加 host-gateway 解析，连接串指向主部署
     在跑库的已发布端口（凭据从源容器 inspect 读取，复用已有测试数据）；
     clone = 任务网络里起与源同镜像的 PG，`pg_dump | psql` 管道灌数据
     （不用 CREATE DATABASE ... TEMPLATE：要求源库零连接，对共享库有风险），
     应用连克隆库随便跑迁移；fresh = 空库。就绪探测一律打到「目标库
     可查」（pg_isready 在 init 临时服务阶段就会假阳性）。
  **口令类变量（PASSWORD/SECRET/TOKEN…）永远不让人填**：复用读源凭据、
  其余场景 crypto/rand 自动生成并留痕进度日志。同任务同 HEAD 缓存推荐
  结果；走分诊同款便宜通道，3 分钟硬上限。**推荐 = 预填，启动仍是人
  拍板**。实测（CR-1454）agent 用 git show HEAD 定位改动应用、识破
  同目录 compose 的 build context 必然失败、env 逐条标 file:line 证据。
