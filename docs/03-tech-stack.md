...
**② 单静态二进制让 D3 的多节点几乎免费。**
新增一台阿里云节点 = `scp lathe` + 跑起来。没有运行时、没有 `node_modules`、没有虚拟环境、没有版本地狱。这是"定位为产品、要能装到任意服务器上"这个要求下最省事的形态。配合 `go:embed` 把 Vue SPA 塞进同一个二进制，**整个产品交付物是一个文件**。
