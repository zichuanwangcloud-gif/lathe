...
func TestRunReproOn_RedSemantics(t *testing.T) {
	...
	res := v.runReproOn(context.Background(), base, src,
		[]ReproTest{{File: "repro.sh", Cmd: []string{"sh", "repro.sh"}}}, true, nil, nil)
	...
	res = v.runReproOn(context.Background(), base, src,
		[]ReproTest{{File: "pass.sh", Cmd: []string{"sh", "pass.sh"}}}, true, nil, nil)
	...
	// 命令不存在 ⇒ error（跑不起来）
	res = v.runReproOn(context.Background(), base, src,
		[]ReproTest{{File: "repro.sh", Cmd: []string{"definitely-not-a-real-binary-lathe"}}}, true, nil, nil)
...
func TestRunReproOn_GreenSemantics(t *testing.T) {
	...
	res := v.runReproOn(context.Background(), dir, dir,
		[]ReproTest{{File: "pass.sh", Cmd: []string{"sh", "pass.sh"}}}, false, nil, nil)
	if res.Status != StatusPassed {
